...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 010 ????? 00100 11", slti   , I, R(rd) = ((sword_t)src1 < (sword_t)imm) ? 1 : 0); // signed
  ...
  INSTPAT("??????? ????? ????? 000 ????? 00000 11", lb     , I, R(rd) = SEXT(Mr(src1 + imm, 1), 8));  // signed
  ...
  INSTPAT("0000001 ????? ????? 011 ????? 01100 11", mulhu  , R, 
    R(rd) = (word_t)(((double_word_t)src1 * (double_word_t)src2) >> MUXDEF(CONFIG_ISA64, 64, 32))); // unsigned * unsigned
  INSTPAT("0000001 ????? ????? 010 ????? 01100 11", mulhsu , R, 
    R(rd) = (word_t)(((double_sword_t)(sword_t)src1 * (double_sword_t)src2) >> MUXDEF(CONFIG_ISA64, 64, 32))); // signed * unsigned
